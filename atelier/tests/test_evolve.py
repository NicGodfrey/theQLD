...
class Round13Export(unittest.TestCase):
    def test_export_zip_contains_board(self):
        ...
        with zipfile.ZipFile(io.BytesIO(blob)) as zf:
            ...
            self.assertIn("board.svg", names)
            self.assertIn("sheet.png", names)
            self.assertIn("sheet.pdf", names)
            self.assertIn("RIGHTS.txt", names)
            threads = json.loads(zf.read("threads.json"))
            self.assertTrue(threads)
            self.assertTrue(threads[0].get("messages"))
            self.assertTrue(zf.read("sheet.png").startswith(b"\x89PNG"))
