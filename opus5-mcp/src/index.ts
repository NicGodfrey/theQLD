...
const STATE_DIR = process.env.OPUS5_MCP_STATE_DIR ?? join(homedir(), ".config", "opus5-mcp");
  ...
  tools: [
    {
      name: "ask_opus5",
      description:
        "向 Claude Opus 5（Cursor Cloud Agent，主模型人设）发送消息并等待完整回复。适合复杂推理、架构分析、代码审查等任务。",
      inputSchema: {
        type: "object",
        properties: {
          message: {
    ...
    },
    {
      ...
      description: "列出当前 Cursor API Key 可用的模型（用于排查 Opus 5 model id）",
      inputSchema: {
        type: "object",
        properties: {},
      },
  ...
  try {
    switch (request.params.name) {
      case "ask_opus5": {
        ...
        const agentId = input.reset
          ? undefined
          : envAgentId || state.agentId;
            ...
            content: [
              {
                ...
                text: JSON.stringify(
                  {
                    hasSession: false,
                    message: "尚无 Opus 5 会话。先调用 ask_opus5。",
                  },
                  null,
                  2,
                ),
          ...
          content: [
            {
              ...
              text: JSON.stringify(
                {
                  ok: true,
                  message: "已清除本地 Opus 5 会话 ID。下次 ask_opus5 会创建新 agent。",
                },
                null,
                2,
              ),
